...
## Curriculum map — Season 7: The Long Reach (the primitives, combined, at scale)

Season 6's finale said, truthfully, that there was nothing narratively lower
left to find — the Substrate was the floor. Season 7 doesn't contradict that;
it changes axis instead of depth: not one ship's internals, but a fleet's,
where the same five-chapters-per-season budget goes to structures that
combine what came before rather than introduce a new primitive outright.
Two picks come straight from the Season 6 roadmap note (segment trees, KMP);
graph coloring, sliding window maximum, and LRU cache round out the
remaining classic interview-prep gaps, chosen so at least one chapter
explicitly composes two earlier chapters' primitives (LRU = chapter 10's
linked list + chapter 11's hash map) rather than standing alone.

| Ch | Mission | Concept | Mechanic that forces the concept | Trials |
|----|--------------------------|--------------------------|-----------|--------|
| 29 | The Long Manifest | Segment trees | 8 stations, 9 operations (updates interleaved with range-sum queries); `valueAt()` is capped at exactly 8 total calls (one per starting value, like ch 26), so answering a query after an update requires a maintained structure, not a re-read | 2 |
| 30 | The Signal Match | KMP string matching | An 18-character signal with 2-3 guaranteed (possibly overlapping) pattern occurrences, generated by embedding the pattern at random positions and filling the rest randomly; `charAt()` is capped at exactly text-length total calls, ruling out any restart-from-scratch naive scan | 2 |
| 31 | The Frequency Map | Graph coloring | 7 stations, retry-generated so no station exceeds degree 3 (guaranteeing 4 colors always suffice for the greedy-by-index algorithm); `setColor()` validates against neighbors immediately rather than deferring to a final check | 2 |
| 32 | The Live Feed | Sliding window maximum | 12 readings, 4-wide window (9 answers); `readingAt()` throws on a second read of the same index, identical budget style to chapter 27's monotonic stack — a deliberate callback, since this chapter's deque generalizes that exact structure | 2 |
| 33 | The Memory Ration | LRU cache | Capacity-3 cache, a hand-authored 10-operation sequence (randomized keys/values, fixed operation shape) engineered to require both a genuine eviction and a GET-driven recency bump to answer correctly | 2 |

Engine note: unlike Season 6, none of these chapters are grid/robot levels —
all five expose an abstract array/graph/cache API, continuing the shift
(started around chapter 21) toward the technique tier's own house style.
Segment trees' and the sliding window's read-budget checks are the same
`bump`/throw pattern as chapters 26 and 27; graph coloring's fail-fast
`setColor()` mirrors chapter 25's fail-fast `reportRedundant()`; LRU's
`answer()`-per-operation report shape mirrors chapter 24's trie queries.
No new engine mechanism was needed — every chapter reuses an existing
verification pattern, which is itself the season's point: composition, not
new primitives. The LRU chapter's reference solution runs a real `Map`
through the same operation sequence at generation time (delete-then-reinsert
for recency, evict the first key at capacity) rather than a hand-traced
expected-output table, so the expected answers can't drift from the actual
semantics being taught.

Shipping this season's wider grids (KMP's 13-column signal, the sliding
window's 12-column feed) meant auditing every chapter's game view with
axe-core individually for the first time, rather than spot-checking chapter
1 — which surfaced two latent, pre-existing violations that had never been
wide enough to trigger before: the tile-index label's color (`#42546d` on
the viewport's dark background) fell short of the 4.5:1 contrast minimum,
and `#viewport` had no keyboard path into its own scrollable region once
content overflowed. Both are one-line fixes (a lighter `.tidx` color,
`tabindex="0"` on `#viewport`) that also apply retroactively to every
earlier chapter, not just Season 7's.
...
The curriculum, tooling, and reference library are all complete: seven
seasons (algorithms, data structures, graphs, techniques, a fifth covering
trees/heaps/greedy/two-pointers/backtracking, a sixth covering
tries/union-find/bit-manipulation/monotonic-stacks/Kruskal's algorithm, and
a seventh covering segment trees/KMP/graph coloring/sliding window
maximum/LRU caches), five practice side missions, a JS primer, step-through
debugging, share links, a first-visit guided tour, an accessibility-audited
UI (0 axe-core violations, verified across every chapter's game view and
every reference-library concept), and a 33-page visualized reference
library where every concept has at least two worked examples, six flagship
concepts have three, and every concept ships runnable sample code in five
languages alongside its pseudocode. Remaining ideas are smaller polish:
...
- A Season 8, if the curriculum needs to grow further — the roadmap is open
  again now that Season 7 spent the "combine what you already have" idea;
  a next season would need its own organizing theme (candidates: string
  algorithms in depth, or a return to graph theory for flow/matching).
